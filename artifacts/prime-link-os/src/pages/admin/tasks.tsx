...
import { useListTasks, useCreateTask, useUpdateTask, useListUsers, useListClients, getListTasksQueryKey } from "@workspace/api-client-react";
...
export default function AdminTasks() {
  ...
  const [form, setForm] = useState({ title: "", description: "", priority: "normal", assignedTo: "", clientId: "", deadline: "" });
  ...
  const handleApprove = (id: number) => {
    updateTask.mutate({ id, data: { status: "approved" } }, {
      onSuccess: () => { invalidate(); toast({ title: "Task approved" }); },
    });
  };

  const getUserName = (id: number | null | undefined) => users?.find(u => u.id === id)?.name ?? "Unassigned";
  const getClientName = (id: number | null | undefined) => clients?.find(c => c.id === id)?.clientName ?? "—";

  if (isLoading) {
    return <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="h-20 bg-white/5 rounded-xl animate-pulse" />)}</div>;
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
          ...
          <form onSubmit={handleCreate}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Task description..." className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none placeholder:text-white/20 resize-none sm:col-span-1" rows={2} />
            </div>
      ...
      <div className="space-y-3">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${statusColors[task.status] ?? "bg-white/5 text-white/30"}`}>{task.status.replace("_", " ")}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-white/30">
                    ...
                    <span>Assigned: <span className="text-white/50">{getUserName(task.assignedTo)}</span></span>
                    {task.deadline && <span>Deadline: <span className="text-white/50">{new Date(task.deadline).toLocaleDateString()}</span></span>}
                  </div>
                ...
                {task.status === "completed" && (
                  <button onClick={() => handleApprove(task.id)} className="flex items-center gap-1.5 text-xs bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 px-3 py-1.5 rounded-lg transition-colors shrink-0">
        ...
        {(tasks ?? []).length === 0 && (
          <div className="text-center py-12 text-white/20 text-sm">No tasks yet. Create the first task.</div>
        )}
      </div>
    </div>
  );
