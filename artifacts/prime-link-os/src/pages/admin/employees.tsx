...
import { useListUsers, useCreateUser, useUpdateUser, useDeleteUser, getListUsersQueryKey } from "@/lib/db";
import { useQueryClient } from "@tanstack/react-query";
import { useToast } from "@/hooks/use-toast";
import { Plus, Trash2, X, Check, UserCircle } from "lucide-react";
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    createUser.mutate(
      { data: form },
      {
        ...
        onError: (err) =>
          toast({
            title: err instanceof Error ? err.message : "Failed to create employee",
            variant: "destructive",
          }),
      }
    );
  ...
  const handleToggleStatus = (id: string, currentStatus: string) => {
    ...
    updateUser.mutate(
      { id, data: { status: newStatus } },
      {
        onSuccess: () => {
          invalidate();
          toast({ title: `Employee ${newStatus}` });
        },
      }
    );
  };
          ...
          <form onSubmit={handleCreate}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <select
                ...
                className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-blue-500/50"
              >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-white/30 uppercase tracking-widest">Online</th>
            ...
            <tbody className="divide-y divide-white/5">
              {nonAdminUsers.map((user) => (
                <tr key={user.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${roleColors[user.role] ?? "bg-white/5 text-white/40"}`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${statusColors[user.status] ?? "bg-white/5 text-white/30"}`}>
                      {user.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className={`w-2 h-2 rounded-full ${user.online ? "bg-emerald-400" : "bg-white/10"}`} />
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="text-xs px-3 py-1 rounded-lg border border-white/10 text-white/40 hover:text-white hover:border-white/20 transition-colors"
                      >
