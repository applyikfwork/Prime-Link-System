...
import { useListMessages, useCreateMessage, useListUsers, getListMessagesQueryKey } from "@/lib/db";
...
export default function ChatPage() {
  ...
  const { data: groupMessages } = useListMessages(
    { isGroup: true },
    { query: { queryKey: ["messages", "group"] } }
  );
  const { data: privateMessages } = useListMessages(
    selectedUser ? { receiverId: selectedUser } : {},
    { query: { enabled: !!selectedUser, queryKey: ["messages", "private", selectedUser] } }
  );
  ...
  const getUserName = (id: string) => {
    ...
    return users?.find((u) => u.id === id)?.name ?? `User #${id.slice(0, 6)}`;
  };
      ...
      },
      {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({
            queryKey: getListMessagesQueryKey({ isGroup: activeTab === "group" }),
          });
        },
      }
    );
      ...
      <div className="flex-1 flex gap-4 min-h-0">
        <div className="w-56 shrink-0 bg-white/[0.03] border border-white/5 rounded-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            ...
            {otherUsers.map((u) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-colors ${
                  activeTab === "private" && selectedUser === u.id
                    ? "bg-blue-500/10 text-blue-400"
                    : "text-white/40 hover:text-white"
                }`}
              >
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${
                    u.online ? "bg-emerald-400" : "bg-white/10"
                  }`}
                />
            ...
            {otherUsers.length === 0 && (
              <p className="text-xs text-white/20 px-3 py-2">No other users yet</p>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 bg-white/[0.03] border border-white/5 rounded-2xl flex flex-col min-w-0">
          <div className="px-6 py-4 border-b border-white/5 flex items-center gap-3">
            ...
            <p className="font-semibold text-white text-sm">
              {activeTab === "group"
                ? "Team Group Chat"
                : `DM: ${users?.find((u) => u.id === selectedUser)?.name ?? "Select a user"}`}
            </p>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-3">
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div>
                    {!isMine && (
                      <p className="text-xs text-white/30 mb-1 ml-1">
                    ...
                    <p
                      className={`text-xs text-white/20 mt-1 ${isMine ? "text-right mr-1" : "ml-1"}`}
                    >
            ...
            {messages.length === 0 && (
              <div className="flex items-center justify-center text-white/20 text-sm h-full min-h-[200px]">
